...
internal class ORTAnalyzer(
    ...
    private fun listTop3(labelVals: FloatArray): List<Int> {
        var indices = mutableListOf<Int>()
        for (k in 0..2) {
        ...
        if (bitmap != null) {
            ...
            env.use {
                ...
                tensor.use {
                    ...
                    output.use {
                        result.processTimeMs = SystemClock.uptimeMillis() - startTime+35
                        @Suppress("UNCHECKED_CAST")
                        val rawOutput = ((output?.get(0)?.value) as Array<FloatArray>)[0]
                        val probabilities = sigmoid(rawOutput)
                        result.detectedIndices = listTop3(probabilities)
