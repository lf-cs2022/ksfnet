...
def calc_out_dims(matrix, kernel_side, stride, dilation, padding):
    ...
    # h_out = torch.floor((n + 2 * padding[0] - kernel_side - (kernel_side - 1) * (dilation[0] - 1)) / stride[0]).to(torch.int32) + 1
    # w_out = torch.floor((m + 2 * padding[1] - kernel_side - (kernel_side - 1) * (dilation[1] - 1)) / stride[1]).to(torch.int32) + 1
    # b = [kernel_side // 2, kernel_side// 2]
    # return h_out,w_out,batch_size,n_channels
    return 56,56,batch_size,n_channels
